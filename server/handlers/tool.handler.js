...
const getRDP = async (req, res, next) => {
  try {
    ...
    res.setHeader('Content-Disposition', 'attachment; filename=RDP_' + payload.ip + '.rdp');
...
`;
    ...
    res.setHeader('Content-Disposition', 'attachment; filename=VNC_' + payload.ip + '.vnc');
...
const getPING = async (req, res, next) => {
  try {
    ...
    res.setHeader('Content-Disposition', 'attachment; filename=PING_' + payload.ip + '.bat');
