  ...
  methods: {
    async onItem(id) {
      try {
        ...
        if (this.IDItem) {
          const item = await this.$store.dispatch('api/branch/findOne', this.IDItem);
          this.item = { ...item };
        } else {
          this.onClean();
        }
      ...
      if (this.$refs.form.validate()) {
        try {
          if (this.IDItem) {
            ...
            await this.$store.dispatch('api/branch/createOne', this.item);
            await this.onUpdate();
    ...
    async onDelete() {
      try {
        if (this.IDItem) {
          await this.$store.dispatch('api/branch/removeOne', this.IDItem);
          await this.onUpdate();
