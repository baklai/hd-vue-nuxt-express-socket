      ...
      <v-card-title class="pt-0">
        ...
        <v-menu offset-y open-on-hover>
          ...
          <v-list flat dense subheader>
            ...
            <v-list-item @click="onDelete()" v-if="$hasScope('api:unit:remove:one')">
              <v-list-item-icon class="mr-1">
  ...
  methods: {
    async onItem(id) {
      try {
        ...
        if (this.IDItem) {
          const item = await this.$store.dispatch('api/unit/findOne', this.IDItem);
          this.item = { ...item };
        } else {
          this.onClean();
        }
      ...
      if (this.$refs.form.validate()) {
        try {
          if (this.IDItem) {
            ...
            await this.$store.dispatch('api/unit/createOne', this.item);
            await this.onUpdate();
    ...
    async onDelete() {
      try {
        if (this.IDItem) {
          await this.$store.dispatch('api/unit/removeOne', this.IDItem);
          await this.onUpdate();
